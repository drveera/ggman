...
3. Convert the `toy.clumped` data.frame to a `ggclumps` object using the `ggmanClumps` function. The column names with the index SNPs (`SNP`) and clumps (`SP2`) have to be specified. 

```{r}
toy.clumps <- ggmanClumps(toy.clumped, index.snp = "SNP", clumps = "SP2") 
```

4. Pass the toy.clumped object to `clumps=` argument of `ggman` function. (Note: This step might take a while when large number of clumps are processed. A progress bar will be shown)
...
An inverted Manhattan plot can be created by inverting the direction of p values of variants with negative beta values (or odds ratio < 1). 

```{r}
ggmanInvert(toy.gwas, snp = "snp", bp = "bp", chrom = "chrom", pvalue = "pvalue", effect = "or", method = "or")
```
